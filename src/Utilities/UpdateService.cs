    ...
    public static class UpdateService {
        ...
        public static async Task<bool> CheckForUpdatesAsync(IWin32Window owner) {
            ...
            if (updateInfo.Version <= currentVersion) {
                AppLogger.Info(
                    $"Application is up to date. Current: {currentVersion}, remote: {updateInfo.Version}."
                );
                return false;
            }
            ...
            if (answer != DialogResult.Yes) {
                AppLogger.Info(
                    $"Application update skipped by user. Current: {currentVersion}, remote: {updateInfo.Version}."
                );
                return false;
            }
        ...
        private static async Task<UpdateInfo> FetchLatestReleaseAsync() {
            ...
            if (installerAsset == null || !IsSafeDownloadUrl(installerAsset.BrowserDownloadUrl)) {
                throw new InvalidDataException(
                    $"Latest GitHub release must contain HTTPS asset '{InstallerAssetName}'."
                );
            }
        ...
        private static Version ParseTagVersion(string tagName) {
            ...
            if (!Version.TryParse(normalizedTag, out var version)) {
                throw new FormatException(
                    $"GitHub release tag '{tagName}' must use v1.0.0.2 format."
                );
            }
        ...
        private static async Task DownloadAndRestartAsync(string downloadUrl) {
            ...
            var tempExePath = Path.Combine(
                Path.GetTempPath(),
                $"YSMInstaller_{Guid.NewGuid():N}.exe"
            );
